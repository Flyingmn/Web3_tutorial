...
contract FundMe {
    ...
    function fund() external payable {
        require(
            transTUsd(msg.value) >= minimumUSD,
            string(
                abi.encodePacked(
                    "Please provide a minimum donation of $",
                    (minimumUSD / 10 ** 18).toString()
                )
            )
        );
    ...
    modifier fundGoalReached() {
        require(
            transTUsd(address(this).balance) >= TARGET,
            "Fund goal has not been reached"
        );
        _;
    }
    //筹资不达标
    ...
    modifier outLimitTime() {
        require(
            block.timestamp >= (deployTime + limit_time),
            "Now is in limit time"
        );
        _;
    }
}
